 ...
 * on the user's machine: Claude Code, Codex, Grok Build, pi, or Cursor Agent. The daemon (daemon.ts) hands
 ...
 *  CUMORA_PI_ARGS / CUMORA_CURSOR_ARGS, space-split). Correctness of the
...
export type EngineId = 'claude' | 'codex' | 'grok' | 'pi' | 'cursor'

/** The pairable engine ids, in the daemon's default detection order. */
export const ENGINE_IDS: EngineId[] = ['claude', 'codex', 'grok', 'pi', 'cursor']
...
export interface EnginePersona {
  ...
  /** Big-brain model pin when known (used for route-qualified Chromatin access). */
  model?: string | null
}
...
/** Exact subscription routes approved for task-bound, read-only Chromatin
 * discovery. Engine identity is material: pi/xAI is not native Grok Build. */
export function grantsChromatinAccess(engine: EngineId, model?: string | null): boolean {
  if (engine === 'claude' || engine === 'codex' || engine === 'grok' || engine === 'cursor') return true
  return engine === 'pi' && typeof model === 'string' && model.startsWith('openai-codex/')
}

/** Project the vault into an eligible agent home. The live daemon is launched
 * under a macOS sandbox that denies writes to the real target; this function
 * owns only link presence and never replaces agent-owned real content. */
...
class ClaudeAdapter implements EngineAdapter {
  ...
  async seedHome(home: string, persona: EnginePersona): Promise<void> {
    ...
    const chromatinAccess = grantsChromatinAccess('claude', persona.model)
...
class CursorAdapter implements EngineAdapter {
  ...
  async seedHome(home: string, persona: EnginePersona): Promise<void> {
    await ensureCommonHome(home)
    const chromatinAccess = grantsChromatinAccess('cursor', persona.model)
    await ensureChromatinLink(home, chromatinAccess)
