...
out({ type: 'result', subtype: 'success', is_error: false, result: 'echo:' + prompt, session_id: sid,
...
test('cursor seedHome writes AGENTS.md and never grants Chromatin', { skip: IS_WIN }, async () => {
  ...
  assert.equal(existsSync(join(f.home, 'chromatin')), false)
})
