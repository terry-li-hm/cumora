...
powered by your **local Claude Code, Codex, pi or Cursor CLI** (BYOA — Bring Your Own
...
Requires **Node ≥ 18** and `claude` (Claude Code), `codex`, `pi` or
`cursor-agent` (Cursor) on your `PATH`. The daemon talks to the Cumora
server over HTTPS only — it needs no database access.
