      ...
      >
        ...
        <div className="px-6 py-5 space-y-4 overflow-y-auto flex-1 min-h-0">
          ...
          <Field
            ...
            hint={isByoa
              ? `Main reasoning model passed to the engine as --model. ${engine === 'codex' ? 'A model name (e.g. gpt-5.5, o3).' : engine === 'pi' ? 'A pi model pattern, ideally provider/id (e.g. anthropic/claude-sonnet-4-6, openai-codex/gpt-5.5); add :<level> to enable thinking.' : engine === 'cursor' ? 'A Cursor model id (e.g. gpt-5, sonnet-4-thinking) or an alias; blank = Auto.' : "A Claude alias or full name (e.g. opus, sonnet, claude-sonnet-4-6)."} Blank = engine default.`
            ...
            <Field
              ...
              hint={engine === 'codex'
                ...
                : engine === 'pi'
                  ...
                  : engine === 'cursor'
                    ? 'Not used by Cursor (it has no separate fast-model knob); triage follows CUMORA_TRIAGE_MODEL on the computer. Blank = fine.'
          ...
          <Field
            label="Runs on"
            hint="Which computer executes this agent. Cumora Cloud is managed; a computer you've paired runs it on your local Claude Code, Codex, pi or Cursor."
          >
            <Select
              ariaLabel="Runs on"
              value={computerId}
              ...
              <div className="mt-2">
                <Select
                  ...
                  options={(selectedComputer.availableEngines.length
                    ...
                    : (['claude'] as EngineId[])
                  ).map((en) => ({ value: en, label: en === 'claude' ? 'Claude Code' : en === 'codex' ? 'Codex' : en === 'pi' ? 'pi' : en === 'cursor' ? 'Cursor' : en }))}
                />
              </div>
            )}
