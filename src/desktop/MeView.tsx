...
const ENGINE_LABEL: Record<string, string> = { managed: 'Cumora', claude: 'Claude Code', codex: 'Codex', grok: 'Grok Build', pi: 'pi', cursor: 'Cursor' }
...
function ComputersTab() {
  ...
  const [engine, setEngine] = useState<'claude' | 'codex' | 'grok' | 'pi' | 'cursor'>('claude')
    ...
    <div className="space-y-6">
      <Section title="↳ Where your agents run">
        <p className="text-[13px] text-ink-500 mb-4 max-w-[640px]">
          ...
          <span className="font-mono text-[12px]"> Grok Build</span>,
        ...
        {code ? (
          <div className="mt-4 bg-sky-50 rounded-[14px] p-4" style={{ border: '1px solid var(--sky-100)' }}>
            ...
            <div className="text-[11.5px] text-ink-500 mb-2.5 italic font-display">
              Needs <span className="font-mono not-italic">claude</span>, <span className="font-mono not-italic">codex</span>, <span className="font-mono not-italic">grok</span>, <span className="font-mono not-italic">pi</span>, or <span className="font-mono not-italic">cursor-agent</span> installed. The computer names itself after that machine and appears here once paired. This token stays valid.
            </div>
            <div className="flex items-center gap-2.5 mb-2.5">
              ...
              <div className="inline-flex rounded-[9px] p-0.5" style={{ background: 'var(--ink-100)' }}>
                {([['claude', 'Claude Code'], ['codex', 'Codex'], ['grok', 'Grok Build'], ['pi', 'pi'], ['cursor', 'Cursor']] as const).map(([id, label]) => (
